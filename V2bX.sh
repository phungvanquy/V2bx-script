...
if [[ -f /etc/redhat-release ]]; then
    ...
    release="alpine"
    echo -e "${red}Script does not support alpine system!${plain}\n" && exit 1
...
if [[ x"${release}" == x"centos" ]]; then
    ...
    if [[ ${os_version} -eq 7 ]]; then
        echo -e "${red}Note: CentOS 7 does not support hysteria1/2 protocol!${plain}\n"
    fi
...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/phungvanquy/V2bx-script/refs/heads/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/phungvanquy/V2bx-script/refs/heads/main/install.sh) $version
    if [[ $? == 0 ]]; then
...
config() {
    echo "V2bX will automatically attempt to restart after modifying the configuration"
    ...
    case $? in
        0)
            ...
            ;;
        1)
            echo -e "Detected that you have not started V2bX or V2bX failed to restart automatically, do you want to view the log? [Y/n]" && echo
...
uninstall() {
    ...
    echo ""
    echo -e "Uninstallation successful. If you want to delete this script, run ${green}rm /usr/bin/V2bX -f${plain} after exiting the script."
    echo ""
...
start() {
    ...
    if [[ $? == 0 ]]; then
        echo ""
        echo -e "${green}V2bX is already running, no need to start again. If you need to restart, please choose restart.${plain}"
    else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        echo -e "${red}V2bX failed to stop. It may be because the stop time exceeded two seconds. Please check the log information later.${plain}"
    fi
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        ...
        echo -e "${red}V2bX failed to set to start on boot.${plain}"
    fi
...
disable() {
    systemctl disable V2bX
    if [[ $? == 0 ]]; then
        echo -e "${green}V2bX disabled from starting on boot successfully.${plain}"
    else
        echo -e "${red}V2bX failed to disable from starting on boot.${plain}"
    fi
...
update_shell() {
    wget -O /usr/bin/V2bX -N --no-check-certificate https://raw.githubusercontent.com/phungvanquy/V2bx-script/refs/heads/main/V2bX.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_uninstall() {
    ...
    if [[ $? != 2 ]]; then
        echo ""
        echo -e "${red}V2bX is already installed, please do not install it again${plain}"
        if [[ $# == 0 ]]; then
            before_show_menu
        fi
        return 1
...
add_node_config() {
    ...
    if [ "$core_hysteria2" = true ] && [ "$core_xray" = false ] && [ "$core_sing" = false ]; then
        ...
        echo -e "${yellow}Please select the node transmission protocol:${plain}"
...
generate_config_file() {
    ...
    while true; do
        if [ "$first_node" = true ]; then
            ...
            read -rp "Do you want to continue adding node configurations? (Press Enter to continue, type n or no to exit)" continue_adding_node
